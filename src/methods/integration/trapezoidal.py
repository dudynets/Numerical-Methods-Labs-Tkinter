...
class TrapezoidalMethodPage(customtkinter.CTkScrollableFrame):
    def __init__(self, master):
        ...
        self.title_label = customtkinter.CTkLabel(
            self,
            text="Метод трапецій (для інтегрування)",
        ...
        self.calculate_button = customtkinter.CTkButton(
            self, text="Розрахувати", command=self.calculate_trapezoidal_method
        )
        self.calculate_button.grid(
            row=5, column=0, columnspan=2, padx=(10, 0), pady=(20, 10), sticky="w"
        )

    def calculate_trapezoidal_method(self):
        plt.close("all")
