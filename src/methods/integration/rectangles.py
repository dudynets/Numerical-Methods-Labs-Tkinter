...
class RectanglesMethodPage(customtkinter.CTkScrollableFrame):
    def __init__(self, master):
        ...
        self.title_label = customtkinter.CTkLabel(
            self,
            text="Метод прямокутників (для інтегрування)",
        ...
        self.calculate_button = customtkinter.CTkButton(
            self, text="Розрахувати", command=self.calculate_rectangles_method
        )
        self.calculate_button.grid(
            row=5, column=0, columnspan=2, padx=(10, 0), pady=(20, 10), sticky="w"
        )

    def calculate_rectangles_method(self):
        plt.close("all")
        ...
        try:
            ...
            dx = (b - a) / n
            # x_values = np.linspace(a, b - dx, n) # left
            x_values = np.linspace(dx / 2, b - dx / 2, n)  # middle
            # x_values = np.linspace(dx, b, n) # right
