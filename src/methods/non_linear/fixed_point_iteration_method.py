...
class FixedPointIterationMethodPage(customtkinter.CTkScrollableFrame):
    def __init__(self, master):
        ...
        self.title_label = customtkinter.CTkLabel(
            self,
            text="Метод простої ітерації (для нелінійних рівнянь)",
        ...
        self.calculate_button = customtkinter.CTkButton(
            self, text="Розрахувати", command=self.calculate_fixed_point_method
        )
        self.calculate_button.grid(
            row=6, column=0, columnspan=2, padx=(10, 0), pady=(20, 10), sticky="w"
        )

    def calculate_fixed_point_method(self):
        plt.close("all")
        ...
        try:
            if not (f_string and x0_str and eps_str and max_iter_str):
            ...
            def implementation(f, x0, eps, max_iter):
                x = x0
                iterations = 0
                ...
                raise ValueError(
                    f"Досягнуто максимальну кількість ітерацій ({max_iter}). Результат: x = {x}"
                )

            result, iterations = implementation(f_np, x0, eps, max_iter)
            ...
            self.plot_function(f_np, x0, result)
    ...
    def plot_function(self, f_np, x0, root):
        try:
